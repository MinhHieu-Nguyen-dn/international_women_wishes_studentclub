...
st.set_page_config(page_title="Kễnh con 98", layout="wide", initial_sidebar_state="expanded")

# Display the title and logo in the same row with adjusted column sizes
col1, col2 = st.columns([1, 3])
with col1:
    st.image("logo.jpg", width=50)  # Adjust the width to your preference
with col2:
    st.title("Nhân ngày Quốc tế Phụ nữ, chúc các em gái 98")

# Split the web into 2 columns with equal width
col3, col4, col5 = st.columns(3)

# Get the list of images
image_list = os.listdir("pics98")
...
while True:
    for wish in wishes:
        ...
        img_1 = col3.image(f"pics98/{image_list[wish_counter % len(image_list)]}", width=200)  # Adjust the width to your preference
        img_2 = col5.image(f"pics98/{image_list[(wish_counter + 1) % len(image_list)]}", width=200)
